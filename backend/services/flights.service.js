...
/* shared SELECT so search + single-flight return the same shape.
   seats_available = total_seats minus seats actually occupied (works even
   before a flight's seats have been lazily generated). */
const FLIGHT_SELECT = `
  SELECT
    f.id,
    f.flight_number,
...
/* ---- on-demand seat generation ----
   We don't pre-seed 180 seats for 42k+ flights. The first time a flight's
   seat map is requested (or it's booked), we create its seats here.
   Rows 1-30, columns A B C / H J K; front 2 rows = premium (business). */
const SEAT_COLS = ['A', 'B', 'C', 'H', 'J', 'K']
const SEAT_ROWS = 30
// premium (purple, +฿1000) rows: front 2 rows + extra-legroom rows around the exits
